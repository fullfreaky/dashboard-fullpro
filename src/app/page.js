                      ...
                      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 10px" }}>
                        ...
                        <ResponsiveContainer width="100%" height={260}>
                          {(() => {
                            ...
                            // Keep 10x ratio: whoever dominates sets both scales
                            const vendasMax = Math.max(maxVen, Math.ceil(maxEst / 10));
                            const estoqueMax = Math.max(maxEst, maxVen * 10);
                            return (
                          <ComposedChart data={merged}>
                            ...
                            {hasEstoque && <YAxis yAxisId="e" orientation="right" stroke={dark ? "#F59E0B" : "#D97706"} fontSize={9} domain={[0, estoqueMax]} allowDataOverflow={false} tickFormatter={(v) => Math.round(v)} />}
